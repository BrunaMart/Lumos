...
async function fetchSeriesBySearch(query) {
    const res = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=${API_KEY}&query=${query}`);
    const data = await res.json();
...
async function fetchSeriesByGenre(genreId) {
    const res = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&with_genres=${genreId}`);
    const data = await res.json();
    displaySeries(data.results);
}
